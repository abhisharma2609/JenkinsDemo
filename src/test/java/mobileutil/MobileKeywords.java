...
public class MobileKeywords {
	...
	/*
	 * //public static Appiumdriver1<MobileElement> driver1=null; public static Appiumdriver1<MobileElement> driver1 =
	 * (Appiumdriver1<MobileElement>) GlobalUtil.getdriver1(); public static Appiumdriver1<MobileElement> driver1 =
	...
	/*
	 * public static boolean tap(String path,String type) { WebElement element = (explicitWaitForElement(path, type));
	 * TouchAction action = new TouchAction((IOSdriver1)driver1); action.tap(element).perform(); return true; }
	 */
	/*
	 * public static void init(Class clazz){ if(!isInit){ normalLogger = Logger.getLogger(clazz+",NormalLogger");
	 * normalLogger.setLevel(Level.INFO); normalFileApp= new FileAppender(); normalFileApp.setLayout(patternLayout);
	 * normalFileApp.setFile(System.getProperty("user.dir")+GetValue( "logInfoFilePath"));
	 * normalFileApp.setImmediateFlush(true); normalLogger.addAppender(normalFileApp); normalFileApp.activateOptions();
	 * errorLogger = Logger.getLogger(clazz+",ErrorLogger"); errorLogger.setLevel(Level.ERROR); errorFileApp= new
	 * FileAppender(); errorFileApp.setLayout(patternLayout); errorFileApp.setFile(System.getProperty("user.dir")+GetValue(
	 * "logErrorFilePath")); errorFileApp.setImmediateFlush(true); errorLogger.addAppender(errorFileApp);
	 * errorFileApp.activateOptions(); conApp = new ConsoleAppender(); conApp.setLayout(consolePatternLayout);
	 * conApp.setTarget("System.out"); conApp.activateOptions(); normalLogger.addAppender(conApp); isInit=true; } }
	 */
	/*
	 * public static void infoLog(Class clazz,String message){ init(clazz); normalLogger.info(message); }
	 */
...
//		testHist.log(Status.PASS, stepName);

	}
	/*
	 * public static void executeStep(Boolean check, Class className, String logstep) { try { if (check) {
	 * infoLog(className, logstep + "-PASS "); stepInfo(logstep + "-PASS"); } else { infoLog(className, logstep + "-FAIL ");
	 * stepInfo(logstep + "-FAIL"); } }catch(Exception e) { throw e; } }
	 */
...
//		test.log(Status.PASS, html);
//		testHist.log(Status.PASS, html);

	}

	public static void stepFail(String stepName) {
		String html = "<span style='color:red'><b>" + stepName + "-FAIL</b></span>";
//		test.log(Status.INFO, html);
//		testHist.log(Status.INFO, html);

	}
	/*
	 * public static void logResult(boolean status, String logStep ){ if (status) { infoLog(MobileKeywords.class, logStep +
	 * "-PASS "); stepPass(logStep); } else { infoLog(MobileKeywords.class, logStep + "-FAIL "); stepFail(logStep); } }
	 */

	/*
	 * public static boolean selectByIndex(String path, String type, Integer index){ boolean status = false; boolean
	 * elemPresent = isWebElementPresent(path, type); if(elemPresent){ WebElement elem =
	 * driver1.findElement(locatortype(type, path)); Select select = new Select(elem); select.selectByIndex(index); status =
	 * true; logResult(status, "Select action is performed !!!" ); return status; }else{ status = false; logResult(status,
	 * "No Select action performed !!!" ); return status; } }
	 */
	 ...
	 * GlobalUtil.getMDriver().manage().window().getSize(); int height = dim.getHeight(); int width = dim.getWidth(); int x
	 * = width / 2; int starty = (int) (height * 0.60); int endy = (int) (height * 0.20); GlobalUtil.getMDriver().swipe(x,
	 * starty, x, endy, 750); }
	 */
	...
	public static void scrollDown() {
		...
		for (int i = 1; i <= 9; i++) {
			...
			// MobileElement element = (MobileElement) GlobalUtil.mdriver.findElements(MobileBy.AccessibilityId("Add to Cart Add to
			// Shopping Cart"));
			...
			if (GlobalUtil.mdriver.findElements(MobileBy.AccessibilityId("Add to Cart Add to Shopping Cart")).size() > 0) {
				...
				// GlobalUtil.mdriver.findElement(By.xpath("//android.widget.Button[@content-desc=\"Add to Cart Add to Shopping
				// Cart\"]")).click();
	...
	/*
	 * public static void swipeVertical(double startPercentage, double finalPercentage) { try { Thread.sleep(2000); } catch
	 * (InterruptedException e) { // TODO Auto-generated catch block e.printStackTrace(); } Dimension size =
	 * GlobalUtil.getMDriver().manage().window().getSize(); int anchor = (int) (size.width * 0.5); int startPoint = (int)
	 * (size.height * startPercentage); int endPoint = (int) (size.height * finalPercentage); new TouchAction(
	 * GlobalUtil.getMDriver()).press((WebElement) PointOption.point(anchor, startPoint)).moveTo((WebElement)
	 * PointOption.point(anchor, endPoint)).release().perform(); }
	 */

	public static void scrollInMobile() throws InterruptedException, IOException {
		/*
		 * executionDelay(1000); try { for (int i = 0; i < 5; i++) { if (isElementDisplayed(element)) { break; } else {
		 * swipeUp(); } } } catch (Exception e) { System.out.println("Scroll to mobile element failed"); }
		 */
	}
	...
	////// ramesh //////
	/*
	 * public static void scrollToMobileElement(MobileElement element, int scrollCount) { try { for (int i = 0; i <
	 * scrollCount; i++) { if (isElementDisplayed(element)) { break; } else { swipeUp(); } } } catch (Exception e) {
	 * System.out.println("Scroll to mobile element failed"); } } public static void swipeUp() { Dimension dim =
	 * GlobalUtil.getMDriver().manage().window().getSize(); int height = dim.getHeight(); int width = dim.getWidth(); int x
	 * = width / 2; int top_y = (int) (height * 0.80); int bottom_y = (int) (height * 0.20);
	 * System.out.println("coordinates :" + x + "  " + top_y + " " + bottom_y); TouchAction ts = new
	 * TouchAction(GlobalUtil.getMDriver()); ts.press(PointOption.point(x, top_y)).moveTo(PointOption.point(x,
	 * bottom_y)).release().perform(); Dimension size = GlobalUtil.getMDriver().manage().window().getSize(); int starty =
	 * (int) (size.height * 0.85); int endy = (int) (size.height * 0.2); int startx = (int) (size.width / 2.2); try {
	 * Thread.sleep(2); System.out.println("Trying to swipe up from x:" + startx + " y:" + starty + ", to x:" + startx +
	 * " y:" + endy); // new TouchAction(GlobalUtil.getMDriver()).press(PointOption.point(startx,
	 * starty)).moveTo(PointOption.point(startx, endy)).release().perform(); new
	 * TouchAction(GlobalUtil.getMDriver()).press(startx, starty).waitAction(2).moveTo(startx, endy).release().perform(); //
	 * reportLogging("Swipe up"); } catch (Exception e) { System.out.println("Swipe action was not successful."); } }
	 */

	/*
	 * public static boolean isElementDisplayed(MobileElement locator) { try { if (locator.isDisplayed())
	 * System.out.println("Element present on screen ***********" + locator); return true; } catch (NoSuchElementException
	 * e) { System.out.println("Element not present on screen **************" + locator); return false; } }
	 */

}

/*
 * int pressX = GlobalUtil.getMDriver().manage().window().getSize().width / 2; // int bottomY =
 * GlobalUtil.getMDriver().manage().window().getSize().height * // 4/5; int bottomY = (int)
 * (GlobalUtil.getMDriver().manage().window().getSize().height * 0.8); // int topY = (int)
 * (GlobalUtil.getMDriver().manage().window().getSize().height // /8); int topY = (int)
 * (GlobalUtil.getMDriver().manage().window().getSize().height * 0.4); int i = 0; do { boolean isPresent; // String
 * XpathElement = AndriodConstants.Amazon.add_to_cart_btn; // isPresent = //
 * GlobalUtil.getMDriver().findElements(By.xpath( "//*[@resource-id='add-to-cart-button']")).size()>0; isPresent =
 * GlobalUtil.getMDriver().findElements(By.xpath(ElementPassed)).size() > 0; if (isPresent) { break; } else {
 * TouchAction touchAction = new TouchAction(GlobalUtil.getMDriver()); touchAction.press(pressX,
 * bottomY).waitAction(Duration.ofMillis(1000)).moveTo(pressX, topY) .release().perform(); // touchAction.press(pressX,
 * bottomY).moveTo(pressX, topY).release().perform(); } i++; } while (i <= 4); }
 */
 ...
 * findElementByXPath("//android.widget.Button[@text='Alexa See how it works']" ); WebElement Selem =
 * GlobalUtil.getMDriver().findElementByXPath( "//*[@resource-id='add-to-cart-button']"); int x =
 * Felem.getLocation().getX(); int y = Felem.getLocation().getY(); int x1 = Selem.getLocation().getX(); int y1 =
 * Selem.getLocation().getY(); GlobalUtil.getMDriver().swipe(x1, y1, x, y, 1);
 */
...
/*
 * Boolean isFoundElement; isFoundElement = GlobalUtil.getMDriver().findElements(By.xpath(
 * "//*[@resource-id='add-to-cart-button']")).size()>0; while (isFoundElement == false){ TouchAction scrollto = new
 * TouchAction(GlobalUtil.getMDriver()); scrollto.press(Felem).waitAction(Duration.ofSeconds(2)).moveTo(Selem).release
 * ().perform(); Dimension dim = GlobalUtil.getMDriver().manage().window().getSize(); int height = dim.getHeight(); int
 * width = dim.getWidth(); int x = height/2; int starty1 = (int)(height * 0.80); int endy1 = (int)(height * 0.20);
 * TouchAction scrollto = new TouchAction(GlobalUtil.getMDriver()); scrollto.longPress()
 * //GlobalUtil.getMDriver().longPress(x,starty1).moveTo(0, endy1).release(); scrollto.perform(); //Thread.sleep(1000);
 * }
 */
/*
